...
class TerrainPatchDatasetTest {
    ...
    void testClear() {
        if (ChunkDataExtractor.isWorldDataAvailable()) {
            ...
            if (initialCount > 0) {
                assertTrue(dataset.isLoaded(), "Dataset should be loaded before clear");
    ...
    void testPatchDataValidation() {
        if (ChunkDataExtractor.isWorldDataAvailable()) {
            ...
            if (dataset.getPatchCount() > 0) {
                ...
                int[][] heightmap = patch.getHeightmap();
    ...
    void testMultipleLoadCalls() {
        if (ChunkDataExtractor.isWorldDataAvailable()) {
            ...
            assertEquals(secondLoad, dataset.getPatchCount(), "Patch count should match last load");
            assertTrue(dataset.isLoaded(), "Dataset should remain loaded after multiple loads");
        }
    }
